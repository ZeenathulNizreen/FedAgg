...
def fl_finetune(
        ...
        data_path: str = '/home/jovyan/work/FedAgg/data/100/',
    ...
    for epoch in tqdm(range(num_communication_rounds)):
        ...
        model_fedavg = FedAvg(model,
                              ...
                              epoch)
        torch.save(model_fedavg.state_dict(), os.path.join(output_dir, str(epoch), "FedAvg_adapter_model.safetensors"))

        # Evaluate Linear model
        print("Evaluating Linear model...")
        linear_loss = evaluate_model(model_linear, tokenizer, os.path.join(data_path, 'global_test.json'))
        print(f"Linear model loss: {linear_loss}")
    ...
    # Save the final aggregated models
    final_fedavg_model_path = os.path.join(output_dir, "final_fedavg_model.safetensors")
...
# merge_models_kit function with support for additional merge methods
import os
import torch
...
def merge_models_kit(model, selected_clients_set, output_dir, epoch, merge_type="linear"):
    ...
    if merge_type == "linear":
        # Define weights for each model
        weights = [1.0] * len(model_weights_list)
        # Perform linear merge
        merged_weights = Linear.merge(model_weights_list, weights=weights)
    
    elif merge_type == "ties":
        # Define the ties merge configuration
        merged_weights = Ties.merge(model_weights_list)
    
    elif merge_type == "passthrough":
        # Define the passthrough merge configuration
        merged_weights = Passthrough.merge(model_weights_list)
    
    elif merge_type == "task_arithmetic":
        # Define the task arithmetic merge configuration
        merged_weights = TaskArithmetic.merge(model_weights_list)
