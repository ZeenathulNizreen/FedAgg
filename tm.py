...
from fed_utils import FedAvg, TaskArithmeticAgg, client_selection, global_evaluation, GeneralClient
import datasets
...
def fl_finetune(
    ...
    for epoch in tqdm(range(num_communication_rounds)):
        ...
        model = TaskArithmeticAgg(model,
                                  selected_clients_set,
                                  output_dir,
                                  epoch)
        torch.save(model.state_dict(), os.path.join(output_dir, str(epoch), "TaskArithmetic_adapter_model.bin"))
        config.save_pretrained(output_dir)
